...
def words(text, reg=re.compile('[a-z0-9]+')):
    ...
    Ex: words("``EGAD!'' Edgar cried.") ==> ['egad', 'edgar', 'cried'] """
    return reg.findall(text.lower())

def canonicalize(text):
    """Return a canonical text: only lowercase letters and blanks.
    Ex: canonicalize("``EGAD!'' Edgar cried.") ==> 'egad edgar cried' """
...
def bigrams(text):
    """Return a list of pairs in text (which is a sequence of letters or words).
    Ex: bigrams('this') ==> ['th', 'hi', 'is']"""
