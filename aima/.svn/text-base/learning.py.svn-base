...
class ReflexLearningAgentProgram:
    """Abstract class for an agent program that keeps track of examples,
    and chooses an action based solely on the current input.
    To subclass this, define hypothesis and induce. [Fig. 18.3]"""
    def __init__(self):
        self.examples = {}

    def __call__(self, percept):
        """A learning agent uses the performance element to get the action,
        and informs the learning element so future actions will be better."""
        action = self.performance_element(percept)
        self.learning_element(percept, action)
        return action

    def performance_element(percept):
        "A reflex learning agent bases action on just the current percept."
        if percept in examples:
            return examples[percept]
        else:
            self.induce()
            return self.hypothesis(percept)

    def learning_element(percept, action):
        examples[percept] = action
...
def parse_csv(input, delim=','):
    ...
    The delim defaults to ','; '\t' and None are also reasonable values.
    Ex: parse_csv('1, 2, 3 \n 0, 2, na') ==> [[1, 2, 3], [0, 2, 'na']]"""
...
class Learner:
    ...
    def predict(self, example): abstract()
...
class NaiveBayesLearner(Learner):
    ...
    def predict(self, example):
        ...
        def class_probability(targetval):
            return reduce(operator.mul, [self.P(targetval, a, example[a])
