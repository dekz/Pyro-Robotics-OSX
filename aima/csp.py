...
def backtracking_search(csp, mcv=False, lcv=False, fc=False, mac=False):
    ...
    Ex: backtracking_search(australia)"""
    if fc or mac:
...
def order_domain_values(var, assignment, csp):
    ...
    if csp.lcv:
        ...
        domain.sort(comparer(lambda val: csp.nconflicts(var, val, assignment)))
    while domain:
        yield domain.pop()
...
def min_conflicts(csp, max_steps=1000000): 
    ...
    Ex: min_conflicts(australia); min_conflicts(usa)"""
...
class UniversalDict:
    ...
    Ex: d = UniversalDict(42); d['life'] ==> 42"""
