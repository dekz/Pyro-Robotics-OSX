...
def pl_fc_entails(KB, q):
    """Use forward chaining to see if a HornKB entails the symbol q. [Fig. 7.14]
    Ex: pl_fc_entails(Fig[7,15], expr('Q')) ==> True"""
...
def find_pure_symbol(symbols, unknown_clauses):
    ...
    Ex: find_pure_symbol([A, B, C], [A|~B,~B|~C,C|A]) ==> A, True"""
    for s in symbols:
...
def find_unit_clause(clauses, model):
    ...
    Ex: find_unit_clause([A|B|C, B|~C, A|~B], {A:True}) ==> B, False"""
...
def literal_symbol(literal):
    ...
    Ex: literal_symbol(P) ==> P; literal_symbol(~P) ==> P"""
    if literal.op == '~':
