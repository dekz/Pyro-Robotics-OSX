...
def Lexicon(**rules):
    """Create a dictionary mapping symbols to alternative words.
        Ex: Lexicon(Art = "the | a | an") ==> {'Art' : ['the', 'a', 'an']}"""    
    for (lhs, rhs) in rules.items():
...
def generate_random(grammar=E_, s='S'):
    ...
    This is useful for testing a grammar.
    Ex: generate_random(E_)"""
    import random
...
class Chart:
    ...
    Ex: chart = Chart(E0); 
    chart.parses('the stench is in 2 2'); len(_) ==> 1
    chart.parses('the wumpus that is smelly is near 2 2'); len(_) ==> 1
